...
## 1. SECTOR DESK (LIVE IMPLEMENTATION - JAN 2026)

### 1.1 Left Navigation Sidebar
- [x] Vertical sidebar with dark theme
- [x] Navigation menu items (Sector, Controls, Enterprise, etc.)
...
- [x] **Dynamic Data Binding**: All data fetched from Supabase (`dashboard_kpis`, `outcomes_data`)
- [x] **Default Filter**: Hardcoded to start at **2025 Q4** (no flicker)

#### 1.2.1 Tab System
- [x] Two tabs: Strategic Impact & Transform Health
- [x] Persists state across renders

#### 1.2.2 Strategic Impact Tab - Radar Chart
- [x] **Engine**: Apache ECharts (High Fidelity)
- [x] **Data Logic**: 
  - [x] Actuals vs Targets fetched from Backend
  - [x] Normalized to 0-100 scale for radar visualization
- [x] **Tooltip**: Custom HTML tooltip with dual values

#### 1.2.3 Strategic Impact Tab - KPI Gauge Cards (Dynamic)
**General Specs:**
- [x] **Height**: Increased aspect ratio (base 190px) for visibility
- [x] **Typography**: Label weight Normal (400), Width 95%
- [x] **Data**: Real-time from `dashboard_kpis` table

**Specific Dimension Logic:**
- [x] **1.0 Strategic Alignment**: Raw Value (Absolute)
- [x] **2.0 Operational Efficiency**: Raw Value (Absolute)
- [x] **3.0 Risk Mitigation**: Raw Value (Absolute)
- [x] **4.0 Investment ROI**: **Normalized** (`Actual / FinalTarget * 100`). Displayed as %.
- [x] **5.0 Active Investor Rate**: Raw Value (Absolute)
- [x] **6.0 Employee Voice**: **Normalized** (`Actual / FinalTarget * 100`). Displayed as %.
- [x] **7.0 Delivery Velocity**: Raw Value (Absolute)
- [x] **8.0 Tech Compliance**: Raw Value (Absolute)

#### 1.2.4 Transform Health Tab - Interaction
- [x] **Radar Chart**: ECharts, showing 8 health dimensions
- [x] **Charts**:
  - [x] **Project Velocity**: Bar Chart (Actual)
  - [x] **Economic Impact Correlation**: Combo Chart (Bar + Line)
  - [x] **Infrastructure Health**: Multi-bar (Coverage vs Quality)
  - [x] **Portfolio Health**: Bubble Chart (Linear Size Scaling based on Budget)

### 1.3 Main Map Area
- [x] **Engine**: Interactive SVG / Canvas Map
- [x] **Markers**: Dynamic Asset placements
- [x] **Zoom**: Auto-zoom to Saudi Arabia bounds on load

#### 1.4 Status & Color Logic (New Standard)
- [x] **Status Formula**: `Deficit % = ((Planned - Actual) / Planned) * 100`
  - [x] Thresholds: < 5% (Green), < 15% (Amber), ≥ 15% (Red).
- [x] **Gauge Visualization**:
  - [x] **Arc Style**: Full Semi-Circle Arc (100% width).
  - [x] **Color**: Dynamically colored by Status (Green/Amber/Red).
  - [x] **Progress Fill**: Removed (Arc acts as a status indicator ring).
...
- [x] **2. Constraint Canvas (vertical stack)**
  - [x] Three blocks showing limits (not editable):
    ...
    - [x] **Risk Constraints** (red header)
      ...
      - [x] All other gauges remain absolute/raw values.
- [x] **Status Calculation Logic (Frontend Override)**:
  - [x] **Formula**: Deviation based on Quarterly Plan (`kpi_planned`).
  - [x] `Deficit % = ((Planned - Actual) / Planned) * 100`
  - [x] **Thresholds**:
    - [x] Deficit < 5% → **GREEN** (On Track/Surplus)
    - [x] Deficit < 15% → **AMBER** (At Risk)
    - [x] Deficit ≥ 15% → **RED** (Critical)
- [x] **Arc Color Logic**:
  - [x] **Value Arc (Fill)**: Dynamically colored using the Status Gradient (Green/Amber/Red).
  - [x] **Background Track**: Neutral Dark (`#1e293b`) for maximum contrast.
  - [x] Replaced previous logic where background was colored and fill was always green.
...
**Completed:** 2025-01-09
## 8. ADMIN & OBSERVABILITY MIGRATION (Round 14)

### 8.1 Migration from Sandbox to Josoor
- [x] **Observability Page**: Migrated from `src/pages/ObservabilityPage.tsx` to `src/app/josoor/views/admin/Observability.tsx`.
- [x] **Admin Settings Page**: Migrated from `src/pages/AdminSettingsPage.tsx` to `src/app/josoor/views/admin/Settings.tsx`.
- [x] **Styles**: Copied `src/pages/ObservabilityPage.css` to `src/app/josoor/views/admin/Observability.css`.
- [x] **Routing**: Updated `JosoorShell.tsx` to lazy load the new components from `views/admin/`.

### 8.2 Dependency Fixes
- [x] **Legacy SettingsDesk**: Fixed broken import path in `src/components/desks/SettingsDesk.tsx` to point to correct relative CSS location.
