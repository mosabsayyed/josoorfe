...
// Import icons (Public Assets)
const factoryIcon = '/assets/d06a8c9edb7a3c4705388fb7f4e1e9dd775f2172.png';
const layersIcon = '/assets/d8f139bc6eb42908fc05a27d7464940450f1ebce.png';
const buildingsIcon = '/assets/bd89c9093f7e2782417c2cf0a6e772f9b40df676.png';
const cityIcon = '/assets/425f09a50feb64694c6b2c3a1203e88b8090d79e.png';
const chipIcon = '/assets/d90d9826020dfd8a2255a0b0a04f11ea09dcb3cc.png';
const mapOutline = '/assets/ab5ac211c3c610f7bdfcd6d4a2384b98355ce7e5.png';
...
export function SaudiMap() {
  ...
  useEffect(() => {
    const fetchSectorData = async () => {
      setIsLoading(true);
      try {
        const data = await getSectorValueChain();
        ...
        if (data && data.nodes) {
          ...
          // TODO: Transform flows similarly
          setMapFlows(ksaData.flows);
        } else {
          console.warn("API connected but returned no compatible data");
  ...
  useEffect(() => {
    const targetViewBox = isZoomedIn && zoomedPlantId
      ? (() => {
        // Fallback to finding in mapPlants first, then ksaData if needed (though ksaData is deprecated for plants)
  ...
  const visiblePlants = showFuture
    ...
    : mapPlants.filter(p => (p.properties?.status === 'operational' || p.properties?.status === 'at-risk') && p.coords && p.coords.length === 2);
    ...
    <div className="saudi-map-container">
      ...
      <div className="map-view-container">
        ...
        >
          ...
          {/* Map Container with fixed viewBox for perfect coordinate stability */}
          {isLoading && <div className="absolute inset-0 flex items-center justify-center text-white z-50 bg-black/50">Loading Sector Data...</div>}
          {error && <div className="absolute inset-0 flex items-center justify-center text-red-500 z-50 bg-black/50">Error: {error}</div>}
