import svgPaths from '../imports/svg-ydduxkvvhk';
...
interface GaugeCardProps {
  ...
  endValue: number;
  scale?: number;
  family?: string;  // Relaxed from '1.0' | '2.0'
  level?: number;   // Relaxed from 1 | 2
}
...
export function GaugeCard({ label, value, status, delta, baseValue, qMinus1, qPlus1, endValue, scale = 1, family = '1.0', level = 1 }: GaugeCardProps) {
  ...
  // Background color for 2.0 family
  const bgColor = family === '2.0' ? '#1A2539' : '#314158';
  ...
  const fontScale = isL2 ? 0.6 : 1; // 60% font sizes for L2

  return (
    <div className="gauge-card-container" style={{
      aspectRatio: `275/${156.588 * heightScale}`,
      transform: `scale(${scale})`
    }}>
      {/* Background */}
      <div className="gauge-card-bg" style={{
        backgroundColor: bgColor,
        borderRadius: isL2 ? '13px' : '22px'
      }}>
        <div aria-hidden="true" className="gauge-card-border" style={{
          borderRadius: isL2 ? '13.5px' : '22.5px'
        }} />
      </div>
      ...
      <p
        ...
        style={{ fontSize: `clamp(${10 * fontScale}px, ${0.9 * fontScale}vw, ${14 * fontScale}px)` }}
      >
        {label}
      </p>
          ...
          <svg className="gauge-svg" fill="none" preserveAspectRatio="none" viewBox="0 0 275 85.7353">
            ...
            <g>
              ...
              <path
                ...
                fill="none"
              />
              {/* Green arc overlay - shows up to actual value */}
              <path
                d={svgPaths.p5e5a640}
                mask={`url(#path-1-inside-gauge-${label.replace(/\s+/g, '_')})`}
                stroke={`url(#paint_green_gauge_${label.replace(/\s+/g, '_')})`}
                strokeWidth="16"
                fill="none"
                strokeDasharray={`${valuePercentage * 4.32} 432`}
                strokeLinecap="butt"
              />
            </g>
          </svg>
