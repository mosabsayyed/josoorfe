...
// Aura proxy — run `python3 aura-proxy.py` from project root
const AURA_PROXY_URL = 'http://localhost:7476';
...
export function ChainTestDeskAura() {
    ...
    const { data: graphData, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            const response = await fetch(`${AURA_PROXY_URL}/db/neo4j/tx/commit`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ statements: [{ statement: query, parameters: { year: selectedYear } }] })
            });

            if (!response.ok) throw new Error(`Aura proxy error: ${response.status}`);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', background: '#0b0f14' }}>
            <div style={{ padding: '6px 16px', borderBottom: '1px solid rgba(56,189,248,0.35)', background: 'rgba(12,16,22,0.92)', flexShrink: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: '9px', color: '#38bdf8', fontWeight: 600, marginLeft: 'auto', padding: '2px 8px', border: '1px solid #38bdf8', borderRadius: '4px' }}>AURA (PRODUCTION)</span>
                ...
                <div style={{ minHeight: '500px', position: 'relative', width: `${zoom * 100}%`, minWidth: '100%' }}>
                    {(isLoading || error || !displayData) && (
                        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center', color: isDark ? '#fff' : '#000' }}>
                            {isLoading && <div>Loading from Aura...</div>}
                            {error && <div style={{ color: '#ef4444' }}>Error: {String(error)}<br/><span style={{ fontSize: '11px', color: '#9ca3af' }}>Make sure aura-proxy.py is running: python3 aura-proxy.py</span></div>}
            ...
            {displayData && (
                <div style={{ maxHeight: '180px', overflow: 'auto', flexShrink: 0, borderTop: '1px solid rgba(56,189,248,0.35)' }}>
